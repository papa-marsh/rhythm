...
- **Skipping depends on schedule type.** Fixed skips record the earlier of the beat's due day and today, and advance from the due day. Relative skips record today and advance from the later of today and the due day, so overdue skips reset the interval without early skips pulling the schedule forward. Both use the original due day, independent of snooze.
- **"Completed on…" permits backdating through today, bounded inclusively by the latest actual completion.** Skips do not constrain the picker: dismissing an occurrence must not prevent recording when another was completed. With no prior completion, there is no lower bound from history. Fixed completion advances from due; relative completion advances from the selected completion day.
...
- **Urgency tiers** (`UrgencyTier`, ordered): `later` (outside grace) → `almost` → `due` → `overdue` (within one grace past due) → `late`. Tier + days-until-due drive row bar color, chip styling (filled for due/overdue/late, tinted for almost, neutral for later), Today's sort (severity desc, closeness asc), and the Today/Later split. The `almost` tier — and the `notify…Almost` fields/settings tied to it — surface to users as **"Upcoming"**. The code identifier is load-bearing (a CloudKit-mirrored `Cadence.notifyAlmost` field and a `defaultNotifyAlmost` UserDefaults key, neither renamable), so the display label diverges from the internal name on purpose; don't "reconcile" them.
...
Why the design is what it is: iOS cannot run app code at midnight, and local notifications are static pre-scheduled payloads with *absolute* badge numbers. So future badge values are pre-computed (they're deterministic — data only changes through user action, which replans) and delivered as silent `.passive` badge-only notifications at midnight on each day the count changes. Reminders carry the badge value for their own fire day so the two systems can't disagree. An optional **daily digest** adds one alert per day, at the user's digest time, summarizing that day's due/overdue beats (copy in `Core/DigestCopy`) — projected over the future like the badge timeline and skipped on days with nothing waiting. Reminders, badge updates, and digests all compete for **iOS's 64-pending-notification cap**, interleaved chronologically: near-term coverage is complete, horizon accuracy degrades gracefully. Replan triggers: every store mutation, app foreground, midnight/timezone tick, and changes to settings baked into content (sound, show-emojis, daily digest).
